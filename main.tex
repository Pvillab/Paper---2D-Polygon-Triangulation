...
\maketitle
\section{Abstract}
    The following document reports the results gathered during the project called "2D Polygon Triangulation" which the main purpose is to triangulate 2-dimensional polygons with 2 different implementations which are using Delaunay-Constrained Polygons and Non-Delaunay ones. After the experimentation phase both implementations are of order $O(n^2)$. The Non-Delaunay polygons are tested with the use of similar polygons but with different amount of vertices.
    
\section{Introduction}
Game figures and models that are visible in a computer screen and pretty much any other polygon can be deconstructed into triangles. This process is called \textbf{triangulation}. In the following project, two different implementation were tested. One implementation uses polygons with Delaunay constraints and another implementation with non-Delaunay polygons.For the triangulation of non-Delaunay polygons, the chosen approach was the Ear Clipping method, for this implementation only 2D and \textbf{simple polygons} are tested. \textbf{Non-simple polygons} are those with one or more vertex are shared among various triangles or edges between two vertices that are intersected by other edge. Also, no additional vertices will be inserted in order to triangulate. The second implementation 
...
In this experiment it can be seen that in the Spider-Man image it test with different quantity of vertices, and according to that the quantity of triangles will be n-2, and because that the algorithm will last more and will increase his response time theoretically.
